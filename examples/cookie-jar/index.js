...
//Don't hardcode like this. Make sure it is at least in an environment variable
const cookieSecret = 'super-secret';

const app = createServer({ cookies: false });
app.use(cookieParser(cookieSecret));

//don't ever give users the same session token
...
app.get('/login', async (req, res) => {
  ...
  //this needs to be signedCookies instead of cookies
...
app.post('/login', async (req, res) => {
  ...
  //adding the session to the user on the login
  if (user) {
    const sessionId = generateSessionId();
    await db.run('INSERT INTO sessions (id, username) VALUES (?, ?)', [
      sessionId, 
      username,//this is the other column for getting the username from the session
    ]);
...
app.get('/profile', async (req, res) => {
  ...
  //kick them out if they don't have a session
  if (!sessionId) {
